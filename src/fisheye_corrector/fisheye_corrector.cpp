...
void FisheyeCorrector::generateMap()
{
	...
	Eigen::Matrix3f R_fisheye_camera = quaternion.toRotationMatrix()*quaternion_axis.toRotationMatrix();
	...
	for (int h = 0; h < Height_; h++)
		...
		float distance_to_original_axies = sqrt(point_fisheye(0)*point_fisheye(0)+point_fisheye(1)*point_fisheye(1));
		float x = point_fisheye(0) *(radius_in_fisheye / distance_to_original_axies);
		float y = point_fisheye(1)*(radius_in_fisheye / distance_to_original_axies);
            //print_value(degree)
		...
		/*if (degree > 90)
        {
            ...
            print_value(radianToDegree(acos(f_camera_/sqrt(radius_in_project*radius_in_project+ f_camera_*f_camera_))))
            print_value(radius_in_project*radius_in_project+ f_camera_*f_camera_)
            print_value(point_in_original_plane(0)*point_in_original_plane(0)+point_in_original_plane(1)*point_in_original_plane(1)+f_camera_*f_camera_)
