...
float FisheyeCorrector::distortion_list(float theta_in)
{
    ...
    return tan(theta_in / (1 + k1 * theta_in_2 + k2 * theta_in_4 + k3 * theta_in_6))*f_camera_*pixelHeight_;
}
...
void FisheyeCorrector::generateMap()
{
	...
	float  radius_in_fisheye = distortion_list(degreeToRadian(angle_between_original_axis)) / pixelHeight_;
