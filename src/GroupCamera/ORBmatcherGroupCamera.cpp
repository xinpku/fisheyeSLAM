...
namespace ORB_SLAM2
{
    ...
    int ORBmatcher::SearchForInitializationGroupCamera(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
    {
        int nmatches=0;
        ...
        for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
        {
            ...
            vector<size_t> vIndices2 = F2.GetFeaturesInAreaSubCamera(kp1.pt.x,kp1.pt.y, windowSize,level1,level1,kp1.class_id);
            //std::cout<<"cam "<<kp1.class_id<<"  vIndices"<<vIndices2.size()<<std::endl;
            if(vIndices2.empty())
                continue;
