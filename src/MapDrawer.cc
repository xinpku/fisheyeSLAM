...
/*    for(set<MapPoint*>::iterator sit=spRefMPs.begin(), send=spRefMPs.end(); sit!=send; sit++)
    ...
    //TODO: wx-debug-test only for test local mapping of group camera
    glBegin(GL_POINTS);
...
void MapDrawer::GetCurrentOpenGLCameraMatrix(pangolin::OpenGlMatrix &M)
{
    ...
    void MapDrawer::GetCurrentOpenGLCameraMatrixSubCamera(pangolin::OpenGlMatrix &M,int cameraID)
    {
        if(!mGroupCameraPose[cameraID].empty())
        {
            cv::Mat Rwc(3,3,CV_32F);
            cv::Mat twc(3,1,CV_32F);
            {
                unique_lock<mutex> lock(mMutexCamera);
                Rwc = mGroupCameraPose[cameraID].rowRange(0,3).colRange(0,3).t();
                twc = -Rwc*mGroupCameraPose[cameraID].rowRange(0,3).col(3);
            }
